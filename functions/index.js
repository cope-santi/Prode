...
app.post("/api/sync", requireAdmin, async (req, res) => {
  try {
    ...
    const range = dateFrom && dateTo ? { dateFrom, dateTo } : buildPostMatchRange(config);
...
function buildPostMatchRange(config) {
  const now = new Date();
...
exports.syncPostMatch = onSchedule(getPostMatchCron(), async () => {
  const config = getConfig();
  const { dateFrom, dateTo } = buildPostMatchRange(config);
  logger.info("Running post-match sync", { dateFrom, dateTo });
  try {
    await syncMatches({ mode: "post_match", dateFrom, dateTo });
  } catch (error) {
    if (String(error.message || "").includes("Sync already running")) {
      logger.warn("Post-match sync skipped: already running.");
      return;
    }
    logger.error("Post-match sync failed", error);
  }
});

function getPostMatchCron() {
  const config = getConfig();
  const minutes = Number.isNaN(config.postMatchSyncMinutes) ? 10 : config.postMatchSyncMinutes;
  const safeMinutes = minutes > 0 && minutes <= 60 ? minutes : 10;
  if (safeMinutes === 60) return "0 * * * *";
  return `*/${safeMinutes} * * * *`;
}
